...
class CustomGLTFLoader extends Loader {
...
class GLTFParser {
    ...
    loadBuffer(bufferIndex) {
        ...
        const validGLTFBufferBeginnings = [
            "data:application/octet-stream;base64,",
            "data:application/gltf-buffer;base64,"
        ]
        const start = validGLTFBufferBeginnings.find(s => bufferDef.uri.substring(0, s.length) === s)
        if (start !== undefined) {
            return new Promise(function (resolve, reject) {
                const base64String = bufferDef.uri.substring(start.length, bufferDef.uri.length)
